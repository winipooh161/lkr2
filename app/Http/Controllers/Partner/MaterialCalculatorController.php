...
class MaterialCalculatorController extends Controller
{
    ...
    public function exportPdf(Request $request)
    {
        $request->validate([
            ...
            'prices' => 'nullable|array', // Добавляем валидацию цен
        ...
        // Получаем цены из запроса или используем сохраненные/дефолтные
        $requestPrices = $request->input('prices', []);
        ...
        foreach ($request->calculations as $calc) {
            ...
            if (isset($materials[$materialId])) {
                ...
                // Получаем цену: сначала из запроса, потом из сессии, потом дефолтную
                $estimatedPrice = 0;
                if (isset($requestPrices[$materialId]) && $requestPrices[$materialId] > 0) {
                    $estimatedPrice = floatval($requestPrices[$materialId]);
                } else {
