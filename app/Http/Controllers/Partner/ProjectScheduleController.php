...
{    /**
    ...
    public function getFile(Request $request, Project $project)
    {
        ...
        // Если файл не существует, автоматически создаем шаблон
        if (!Storage::disk('public')->exists($filePath)) {
            try {
                Log::info('Файл графика не найден, создаем автоматически', [
                    'project_id' => $project->id,
                    'file_path' => $filePath
                ]);
                
                // Создаем шаблон автоматически
                $response = $this->createTemplate($request, $project);
                $responseData = $response->getData(true);
                
                if (!$responseData['success']) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Не удалось создать шаблон: ' . $responseData['message']
                    ], 500);
                }
                
                Log::info('Шаблон графика автоматически создан', [
                    'project_id' => $project->id
                ]);
                ...
                return response()->json([
                    'success' => false,
                    'message' => 'Файл не найден и не удалось создать шаблон'
                ], 404);
            }
        }
    ...
    public function createTemplate(Request $request, Project $project)
    {
        ...
        try {
            ...
            // Устанавливаем заголовки (такие же как в frontend)
            $headers = [
                'Наименование',
                'Статус',
                'Начало',
            ...
            // Создаем полный план-график работ (такой же как в frontend)
            $sampleData = [
