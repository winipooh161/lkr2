...
class EstimateItemController extends Controller
{
    ...
    public function addRow(Request $request, Estimate $estimate)
    {
        ...
        // Обновляем total_amount и сохраняем смету
        $estimate->total_amount = $totalAmount;
        $estimate->save();
        
        // Сбрасываем кэш, чтобы быть уверенными, что у нас свежие данные из БД
        \Illuminate\Support\Facades\DB::flushQueryLog();
        
        // Проверяем и обновляем статус сметы, если необходимо
        $estimate->ensureValidStatus();
        
        // Перезагружаем модель из базы для гарантированно актуальных данных
        $estimate = Estimate::findOrFail($estimate->id);
    ...
    /**
     * Удаляет элемент сметы
     * 
     * @param EstimateItem $item Элемент сметы
    ...
    public function destroyItem(EstimateItem $item)
    {
        ...
        try {
            // Сохраняем ID сметы перед удалением элемента
            ...
            $totalAmount = EstimateItem::where('estimate_id', $estimateId)
                ->sum('client_cost');
            ...
            return response()->json([
                'success' => true,
                'message' => 'Элемент сметы успешно удален',
                'total_amount' => $totalAmount
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Ошибка при удалении элемента сметы: ' . $e->getMessage()
            ], 500);
        }
    }
}
