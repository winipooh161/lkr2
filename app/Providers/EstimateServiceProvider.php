...
class EstimateServiceProvider extends ServiceProvider
{
    ...
    public function register()
    {
        ...
        // Регистрируем новые сервисы для работы с JSON данными
        $this->app->singleton(EstimateJsonTemplateService::class, function ($app) {
            return new EstimateJsonTemplateService();
        });

        $this->app->singleton(EstimateJsonExportService::class, function ($app) {
            return new EstimateJsonExportService(
                $app->make(EstimateJsonTemplateService::class)
            );
        });
    }
