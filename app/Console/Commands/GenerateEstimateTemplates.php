...
class GenerateEstimateTemplates extends Command
{
    ...
    private function setupWorkHeader($sheet, $title)
    {
        ...
        // Находим последнюю строку для добавления ИТОГО
        $lastRow = $sheet->getHighestDataRow();
        
        // Добавляем итоговую строку в конец
        $sheet->setCellValue('B' . $lastRow, 'ИТОГО:');
        $sheet->setCellValue('F' . $lastRow, '=SUM(F7:F' . ($lastRow - 1) . ')');
        $sheet->setCellValue('J' . $lastRow, '=SUM(J7:J' . ($lastRow - 1) . ')');
    }
    ...
    private function addStandardMaterialExamples($sheet)
    {
        ...
        // Добавляем итоговую строку всегда в конце
        $totalRow = $row; // Последняя строка после всех материалов
        
        // Добавляем итоговую строку
        $sheet->setCellValue('B' . $totalRow, 'ИТОГО:');
        $sheet->setCellValue('F' . $totalRow, $totalMaterialsCost);
        $sheet->setCellValue('J' . $totalRow, $totalClientCost);
