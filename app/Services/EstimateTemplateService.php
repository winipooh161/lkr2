...
class EstimateTemplateService
{
    ...
    private function createMainTemplate(Spreadsheet $spreadsheet)
    {
        ...
        // Добавляем итоговую строку всегда в конце после всех работ
        // Это важно - строка ИТОГО всегда должна быть последней
        $totalRow = $row;
        
        $sheet->setCellValue('A' . $totalRow, '');
        $sheet->setCellValue('B' . $totalRow, 'ИТОГО:');
        $sheet->setCellValue('F' . $totalRow, '=SUM(F7:F' . ($totalRow - 1) . ')');
    ...
    private function createAdditionalTemplate(Spreadsheet $spreadsheet)
    {
        ...
        // Убираем неправильно размещенную итоговую строку и продолжаем добавлять данные
        // Добавляем примеры дополнительных работ
            $itemNumber = 1;
            $row = 7; // Начинаем с 7-й строки для данных
        ...
        // Добавляем итоговую строку всегда в конце после всех работ
        // Сохраняем позицию итоговой строки
        $totalRow = $row;
    ...
    private function createMaterialsTemplate(Spreadsheet $spreadsheet)
    {
        ...
        // Добавляем итоговую строку всегда в конце после всех материалов
        // Это важно - строка ИТОГО всегда должна быть последней
        $totalRow = $row;
