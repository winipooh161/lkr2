            ...
            <div class="modal-body">
                <form id="addItemForm">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="add-total-amount" name="total_amount" min="0" step="0.01">
                    </div>
                    
                    <div class="mb-3">
                        <label for="add-paid-amount" class="form-label">Оплачено (₽)</label>
                        <input type="number" class="form-control" id="add-paid-amount" name="paid_amount" min="0" step="0.01">
                    </div>
...
<script>
    ...
    // Объявляем функцию loadFinanceData в глобальной области видимости
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('saveAddItemBtn').addEventListener('click', function() {
            ...
            // Проверяем только поле наименования, которое обязательно
            ...
            if (isValid) {
                ...
                // Проверяем и обрабатываем пустые числовые поля
                const totalAmountField = document.getElementById('add-total-amount');
                const paidAmountField = document.getElementById('add-paid-amount');
                
                // Устанавливаем 0 для пустых числовых полей
                if (!totalAmountField.value.trim()) {
                    formData.set('total_amount', '0');
                }
                
                if (!paidAmountField.value.trim()) {
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editItemForm">
                    ...
                    <div class="mb-3">
                        <label for="edit-total-amount" class="form-label">Сумма (₽)</label>
                        <input type="number" class="form-control" id="edit-total-amount" name="total_amount" min="0" step="0.01">
                    </div>
                    
                    <div class="mb-3">
                        <label for="edit-paid-amount" class="form-label">Оплачено (₽)</label>
                        <input type="number" class="form-control" id="edit-paid-amount" name="paid_amount" min="0" step="0.01">
                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Обработчик сохранения нового элемента был перемещен в функцию валидации формы выше
        ...
        document.getElementById('saveEditItemBtn').addEventListener('click', function() {
            ...
            // Проверяем и обрабатываем пустые числовые поля
            const totalAmountField = document.getElementById('edit-total-amount');
            const paidAmountField = document.getElementById('edit-paid-amount');
        ...
        // Эти функции теперь определены выше в глобальной области видимости
        // Этот блок комментария указывает, что функции displayFinanceItems, showAlert, getTypeFromTabId и getTableIdFromType
        // теперь определены в начале скрипта, чтобы избежать ошибок "is not defined"
        
        // Вызываем загрузку финансовых данных при загрузке страницы
        loadFinanceData();
    });
</script>
