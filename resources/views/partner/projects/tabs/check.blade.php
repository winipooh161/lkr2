...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initializeCheckHandlers(projectId, checkId) {
        ...
        if (commentTextarea) {
            let typingTimer;
            const doneTypingInterval = 1000; // время в мс
            let previousValue = commentTextarea.value;
            ...
            // Сохранение при нажатии Ctrl+Enter или Cmd+Enter
    ...
    function saveComment(projectId, checkId, comment) {
        // Добавляем визуальный индикатор сохранения
        const commentArea = document.getElementById(`comment${checkId}`);
        const saveIndicator = document.createElement('div');
        saveIndicator.className = 'text-muted small mt-2 save-indicator';
        saveIndicator.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i> Сохранение...';
        
        // Добавляем индикатор после текстового поля
        if (commentArea && commentArea.parentNode) {
            // Удаляем предыдущий индикатор, если он есть
            const oldIndicator = commentArea.parentNode.querySelector('.save-indicator');
            if (oldIndicator) {
                oldIndicator.remove();
            }
            commentArea.parentNode.appendChild(saveIndicator);
        }
        ...
        .then(data => {
            if (saveIndicator) {
                if (data.success) {
                    saveIndicator.innerHTML = '<i class="fas fa-check-circle text-success me-1"></i> Комментарий сохранен';
                    setTimeout(() => {
                    ...
                    saveIndicator.innerHTML = '<i class="fas fa-exclamation-circle text-danger me-1"></i> Ошибка при сохранении';
                    saveIndicator.classList.remove('text-muted');
                    saveIndicator.classList.add('text-danger');
                }
            }
        })
        .catch(error => {
